...
class EmmMotor:
    ...
    def emm_v5_stop_now(self, addr=None, snF=False):
        ...
        cmd = bytes([addr, 0xFE, 0x01 if snF else 0x00, 0x6B])
        self._send_cmd(cmd)
    ...
    def emm_v5_modify_ctrl_mode(self, addr=None, svF=False, ctrl_mode=0):
        ...
        """
        addr = self.motor_id if addr is None else addr
        cmd = bytes([addr, 0x46, 0x01 if svF else 0x00, ctrl_mode, 0x6B])
        self._send_cmd(cmd)
    ...
    def emm_v5_pos_control(
        self, addr=None, snF=False, raF=False, dir=0, vel=0, acc=0, clk=0
    ):
        """
        位置控制模式
        协议格式
    ...
    def get_current_position_angle(self, addr=None):
        ...
        # 获取脉冲数
        pos_int = struct.unpack(">I", data[3:7])[
            0
        ]  # data[3:7] 取出第4到第7个字节，'>I' 表示大端序无符号整数
    ...
    def emm_v5_move_to_angle(
        self, addr=None, angle_deg=0.0, vel_rpm=0, acc=0, abs_mode=False
    ):
        """
        按角度移动电机
        ...
        self.emm_v5_pos_control(
            addr=addr, dir=dir, vel=vel_rpm, acc=acc, clk=clk, raF=abs_mode, snF=False
        )
...
if __name__ == "__main__":
    ...
    motor = None
    # 1. 初始化电机对象
    motor = EmmMotor(port=SERIAL_PORT, baudrate=BAUDRATE, timeout=1, motor_id=MOTOR_ID)
    print("串口连接成功！")
